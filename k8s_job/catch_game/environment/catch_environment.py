  ...
  def Process(self, request_iterator, context):
    ...
    for request in request_iterator:
      ...
      try:
        ...
        logging.info(f"received message: {message_type}")
...
def main(_):
  ...
  servicer = CatchEnvironmentService()
  dm_env_rpc_pb2_grpc.add_EnvironmentServicer_to_server(servicer, server)

  port = server.add_insecure_port('0.0.0.0:' + port_str)
  server.start()
  while not servicer.IsDestroyed():
    logging.info(f"server is running on port={port}, isDone={servicer.IsDestroyed()}")
    time.sleep(1)
  server.stop(None)
